...
select c.id_conferenza FROM conferenza c where titolo ='International Space Expo' and inizio='2023-08-03 8:00' and fine='2023-08-05 20:00' and id_sede=5 into conferenza_id;
if (not found) then
    rollback;
end if;
...
-- Aggiunta sessioni
sessione1_id = add_new_sessione('Exploring Mars: The Future of Human Colonization','2023-08-03 10:00','2023-08-03 13:00',13,conferenza_id,1);
sessione2_id = add_new_sessione('Space Tourism: A New Era of Extraterrestrial Travel','2023-08-03 13:30','2023-08-03 17:30',14,conferenza_id,2);
sessione3_id = add_new_sessione('Satellite Constellations: Revolutionizing Global Connectivity','2023-08-04 10:30','2023-08-04 12:30',15,conferenza_id,1);
sessione4_id = add_new_sessione('Sustainable Space Exploration: Protecting Celestial Bodies and Earth','2023-08-04 13:30','2023-08-04 19:30',13,conferenza_id,2);
sessione5_id = add_new_sessione('Innovations in Space Research: From Astrophysics to Exoplanet Discoveries','2023-08-05 9:30','2023-08-05 20:00',14,conferenza_id,1);

-- Specifica programma sessione 1
select id_programma from programma where id_sessione = sessione1_id into programma1_id;
call add_evento('Opening Event',programma1_id,'15 minutes');
call add_intervento('Advancements in Mars Rover Technology',
...
call add_intervento('The Psychological Challenges of Long-Duration Space Missions',
